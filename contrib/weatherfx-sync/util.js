...
// remove the temperature from the message string (strip HTML first for DSLF / div-wrapped chat).
export function removeTemperature(string) {
    if (string === null || string === "") {
        return false;
    }
    string = string.toString();
    const plain = removeTags(string);
    const stripped = plain === false ? string : plain;
